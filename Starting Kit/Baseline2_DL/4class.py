        ...
        if(lno%10==0):
            ...
            for index in top4:
                if int(labels[index]) == 1:
                    ...
                    label_1_index = index
                    del label_0_indices[j]
                    break
                j+=1
            if is_label_1_present:
                fw.write(qid+"\t"+Query+"\t"+passages[label_1_index]+"\t"+"0"+"\t"+passage_id[label_1_index]+"\n")
                it = 1
                for index in label_0_indices:
                    fw.write(qid+"\t"+Query+"\t"+passages[index]+"\t"+str(it)+"\t"+passage_id[index]+"\n")
                    it+=1
            else:
                it = 1
                for index in top4[1:]:
                    fw.write(qid+"\t"+Query+"\t"+passages[index]+"\t"+str(it)+"\t"+passage_id[index]+"\n")
                    it+=1
                fw.write(qid+"\t"+Query+"\t"+passages[label1]+"\t"+"0"+"\t"+passage_id[label1]+"\n")
            tempscores=[]
            labels = []
            passages = []
            passage_id = []
