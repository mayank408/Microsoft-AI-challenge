...
def cnn_network(queryfeatures, passagefeatures, num_classes):
    with C.layers.default_options(activation=C.ops.relu, pad=False):
        ...
        model   = C.layers.Dense(num_classes, activation=C.softmax,name="overall")(mergeQP) #outupt : 3 
...
def TrainAndValidate(trainfile):
    ...
    for epoch in range(total_epochs):       # loop over epochs
        ...
        metrics = precision_recall_fscore_support(np.array(validation_labels), np.array(predicted_labels), average='weighted')
...
def GetPredictionOnEvalSet(model,testfile,submissionfile):
    ...
    for line in f:
        ...
        score = model(queryVec,passageVec)[0][0] # do forward-prop on model to get score
