...
def model(embedding_matrix):
	...
	model_glove.add(Embedding(vocab_size, 50, input_length=max_query_words+max_passage_words, weights=[embedding_matrix], trainable=False))
	model_glove.add(Bidirectional(LSTM(max_query_words+max_passage_words, dropout_U = 0.2, dropout_W = 0.2, return_sequences=True), merge_mode='concat'))
	model_glove.add(AttentionDecoder(max_query_words+max_passage_words, 50))
	model_glove.add(Flatten())
	model_glove.add(Dense(num_classes, activation='softmax'))
	model_glove.compile(loss='categorical_crossentropy', optimizer=optimizers.Adam(lr=0.0001), metrics=['accuracy'])
...
# 	print(len(word_index))
	...
	for line in f:
		...
		while len(input_vector) < max_query_words:
		...
		for word in words:
			if word in sw:
				continue
			input_vector.append(word_index[word])
		while len(input_vector) < max_passage_words+max_query_words:
			input_vector.append(word_index["zerovec"])
		input_to_bilstm.append(np.array(input_vector))
	f.close()
	#print(np.array(input_to_bilstm).shape)
	n_batch = 10000
	model_bilstm = model(embedding_matrix)
	model_bilstm.fit(np.array(input_to_bilstm),np_utils.to_categorical(labels, num_classes), batch_size=n_batch, validation_split=0.1, epochs = 1)
	...
	for line in f:
		...
		for word in words:
			...
			if word in word_index:
				...
				input_vector.append(word_index["zerovec"])
		while len(input_vector) < max_query_words+max_passage_words:
			input_vector.append(word_index["zerovec"])
	...
	# print(len(queries))
	print(np.array(eval_input).shape)
