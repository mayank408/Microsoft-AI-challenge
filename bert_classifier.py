...
def readQueries(input_file, output_file, is_type = 'train'):
    """
    ...
    samples = [
        trainSample(
            ...
            query = token[1],
            ans = token[2],
            label = int(token[3]) if is_type =='train' else None
        ) for token in tokens # we skip the line with the column names
    ]
    return samples
...
def main():
    ...
    data_dir = 'place/where/datafile/is/located'
    datafile = 'data.tsv' # eval.tsv if is_type = 'eval' [and] data.tsv if is_type = 'train'
    bert_model = 'bert-base-uncased'
    max_seq_length = 128 # (query + passage) truncated after this length and padded if less than this length
    output_dir = 'tmp/bertcheck/'
    train_batch_size = 32
    eval_batch_size = 8
    is_type = 'train' # can be eval, train, test
    ...
    if not is_type='train' and not is_type='eval':
        raise ValueError("At least one of `train` or `eval` must be the value of is_type.")
    ...
    if is_type=='train':
        train_samples = readQueries(os.path.join(data_dir, datafile), is_type = 'train')
        num_train_steps = int(
    ...
    if is_type=='eval' and (local_rank == -1 or torch.distributed.get_rank() == 0):
        eval_examples = readQueries(os.path.join(data_dir, datafile), is_type = 'eval')
